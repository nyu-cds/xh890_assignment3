"""
    N-body simulation.
    Optimize by replacing global variables by local variables
    Runtime: 119.35493760299869
"""
...
def advance(dt):
    ...
    # Set up local state
    compute_deltas_cp = compute_deltas
    update_vs_cp = update_vs
    update_rs_cp = update_rs
    ...
    for body1 in BODIES.keys():
        for body2 in BODIES.keys():
            if (body1 != body2) and not (body2 in seenit):
                ...
                (dx, dy, dz) = compute_deltas_cp(x1, x2, y1, y2, z1, z2)
                update_vs_cp(v1, v2, dt, dx, dy, dz, m1, m2)
                seenit.append(body1)
        
    for body in BODIES.keys():
        (r, [vx, vy, vz], m) = BODIES[body]
        update_rs_cp(r, dt, vx, vy, vz)
...
def report_energy(e=0.0):
    ...
    # Set up local state
    compute_deltas_cp = compute_deltas
    compute_energy_cp = compute_energy
    ...
    for body1 in BODIES.keys():
        for body2 in BODIES.keys():
            if (body1 != body2) and not (body2 in seenit):
                ...
                ((x2, y2, z2), v2, m2) = BODIES[body2]
                (dx, dy, dz) = compute_deltas_cp(x1, x2, y1, y2, z1, z2)
                e -= compute_energy_cp(m1, m2, dx, dy, dz)
                seenit.append(body1)
...
def nbody(loops, reference, iterations):
    ...
    # Set up local state
    report_energy_cp = report_energy
    advance_cp = advance

    for _ in range(loops):
        for _ in range(iterations):
            advance_cp(0.01)
        print(report_energy_cp())
